    ...
    <div className="mx-auto grid max-w-6xl gap-6 px-6 pb-24 sm:px-8 md:grid-cols-2">
      {posts.map((post) => (
        <Link
          key={post.slug}
          href={`/blog/${post.slug}`}
          className="group rounded-2xl border border-gray-100 bg-white p-8 shadow-soft transition duration-300 hover:-translate-y-1 hover:shadow-plush active:scale-[0.99]"
        >
    ...
    <main className="min-h-screen bg-canvas pt-32">
      <section className="mx-auto max-w-6xl px-6 pb-14 sm:px-8">
        ...
        <h1 className="max-w-4xl font-serif text-[clamp(2.5rem,5.5vw,4.8rem)] leading-[1]">
          Guidance for confident global applications.
        </h1>
      </section>
